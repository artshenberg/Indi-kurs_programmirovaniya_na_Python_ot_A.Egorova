...
Отдел кадров просит вас создать словарь employees_data, в котором, ключами будут идентификаторы, а значениями - имена
работников. При этом отдел кадров просит соотнести идентификатор работника с именем следующим образом:

Выбирается самый маленький идентификатор из списка identifiers
Выбирается первое имя по алфавиту из списка employees
Создается пара ключ-значение. В нашем случае самый маленький идентификатор 5, первое имя по алфавиту будет Anfisa.
Значит создаем пару 5: 'Anfisa'
