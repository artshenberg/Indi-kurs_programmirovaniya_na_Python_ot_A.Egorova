"""
Ваша задача написать функцию get_domain_name, которая принимает строку url, извлекает из нее доменное имя и возвращает
его в качестве строки
...
def get_domain_name(url):
    start = url.index('/') + 2 if 'www' not in url else url.index('w') + 4
    end = url.index('.') if 'www' not in url else url.rfind('.')
