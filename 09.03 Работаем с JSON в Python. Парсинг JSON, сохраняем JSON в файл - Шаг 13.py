...
def print_sorted_json_string(string: str) -> None:
    """Сортирует json-данные по возрасту, а при равенстве возраста - в алфавитном порядке"""

    json_string = json.loads(string)
    result = sorted(json_string, key=lambda x: (x.get('age', 0), x.get('name', ''), x.get('country', '')))

    [print(*person.values(), sep=', ') for person in result]


print_sorted_json_string(people)
